...
class MobileAndOldPageRedirector extends Extension {
	...
	static public function find_old_page($params, $parent = null, $redirect = false) {
		...
		if ($page && $page->canView()) {
			...
			if($page->SubsiteID == 0 && $page->DisableMobileRedirect){
				$config = SiteConfig::current_site_config();
				$config->FullSiteDomainNormalized;
				// 			$res = new SS_HTTPResponse();
				return Controller::join_links($config->FullSiteDomainNormalized, $page->URLSegment);
				// 			Debug::show(Controller::join_links($config->FullSiteDomainNormalized, $page->URLSegment));
			}
